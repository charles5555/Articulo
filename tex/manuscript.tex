...
\abstract{%
A novel framework is proposed in order to describe the time-evolution of diluted classical
and quantum gases, initially out of equilibrium and with spatial inhomogeneities, towards
equilibrium. Briefly, we divide the system into small cells, and consider the local equilibrium
hypothesis. We subsequently define a global functional that is the sum of cell $H$-functionals.
Each cell functional recovers the corresponding Maxwell-Boltzmann, Fermi-Dirac, or Bose-Einstein
distribution function, depending on the classical or quantum nature of the gas. The time-evolution
of the system follows the relation $dH/dt\geq0$, and the equality condition occurs if the system
is in the equilibrium state. \textit{Via} the variational method, proof of the previous relation,
which might be an extension of the $H$-theorem for inhomogeneous system, is presented, for both
classical and quantum gases. Furthermore, the $H$-functionals are in agreement with the
correspondence principle, it is discussed how the $H$-functionals can be identified with the system's
entropy, and the relaxation processes of out-of-equilibrium systems are analyzed.%
}
...
\acknowledgments{CMP acknowledges Conacyt PhD scholarship (Registration Number: {\color{red}XXXXXX}).}
...
$f_B=f(\vec v)$ & The Maxwell-Boltzmann distribution function\\
$\mathcal{H}'$  & Our $H$-functional for a classical diluted gas \\
$f'_M=f'(\vec r_M,\vec v,t)$ & The classical distribution function of a cell centered at $\vec r_M$\\
$H_T$ & The $H$-functional proposed by Tolman\\
$\mathcal{H}$  & Our $H$-functional for a quantum diluted gas\\
$f_{nM}=f_M(\vec r_M,\epsilon_n,t)$ & The quantum distribution function of a cell centered at $\vec r_M$\\
\end{tabular}}
%\end{comment}
