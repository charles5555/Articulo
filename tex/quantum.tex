...
total energy $E$, and total number of quantum particles $N$, which can be fermions or
bosons. For our purposes, the volume $V$ is divided into $K$ small cells, each of which
has a constant volume $\delta V_M=V/K$, a temperature $T_M$, a local energy $\epsilon_M$,
a local number of particles $\mathcal{N}_M$, and a local set of distribution functions,
$\{f_{Mn}(t)\}$. Each $f_{Mn}$ $\big(\equiv f(\vec r_M,\epsilon_{n},t)\big)$ represent the number
of particles contained in the $M$-th cell that occupy the energy level $\epsilon_n$.
We propose the following functional as an alternative $H$-functional
for quantum non-homogeneous diluted gases:
%
\begin{eqnarray}
    \mathcal{H} (t)&=&\sum_{M=1}^{K} \sum_{n} [ f_{Mn}(\epsilon_{n},t) \ln f_{Mn}(\epsilon_{n},t)\nonumber \\
...
\color{blue}\commr{Also, the energy spectrum will be divided into $\delta \epsilon_n$ volumes}. We will use
...
variational functional, $\mathcal{H}_M$, local number of particles, $\mathcal{N}_M$, and
...
%
\begin{eqnarray}
    \mathcal{H}_M &=&  \sum_{n} [ f_{Mn}(\epsilon_{n},t) \ln f_{Mn}(\epsilon_{n},t)\nonumber \\
...
Maximizing the variational entropy $\mathcal{H}$ using the Lagrange multipliers method,
we obtain
%
\begin{eqnarray}
