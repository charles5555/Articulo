...
\end{equation}
%
Here $\vec r_M$ is the center of the cell of index $M$, and $f_M(\vec v,t)\neq f_N(\vec v,t)$
...
but in such a manner that it is inteuitively not too far from equilibrium,
...
%
    ...
    \end{equation}
    %
Here, we have used $\bar{f}(\vec{v})$ as defined in Eq.~(\ref{eq:fbardef}).
...
too far away from equilbrium, it is reasonable to expect on the one hand that
